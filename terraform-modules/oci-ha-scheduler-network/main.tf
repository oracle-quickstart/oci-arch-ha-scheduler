...
locals {
  ...
  # general network security groups for OCI HA Scheduler
  oci-ha-scheduler-nsg = { "${var.names_prefix}-nsg" = {
    "${var.names_prefix}-nsg" = {
      ...
      ingress_rules = [
        ...
        },
        {
          ...
          src         = var.oci_ha_scheduler_subnet_cidr
          src_type    = "CIDR_BLOCK"
          src_port    = null
          dst_port    = null
          icmp_code   = null
      ...
      egress_rules = [
        ...
        },
        {
          ...
          stateless   = false
          protocol    = local.icmp_protocol
          dst         = var.oci_ha_scheduler_subnet_cidr
          dst_type    = "CIDR_BLOCK"
          src_port    = null
          dst_port    = null
          icmp_code   = null
  ...
  # general network security groups for util OCI HA Scheduler VM

  oci-ha-scheduler-util-nsg = { "${var.names_prefix}-util-nsg" = var.provision_util_node == false ? {} : {
...
module "oci_network" {
  ...
  route_tables = {
    "${var.names_prefix}_route_table" = {
      ...
      defined_tags   = var.defined_tags
      freeform_tags  = var.freeform_tags
      route_rules    = var.oci_ha_scheduler_route_table.route_rules
    }
  }
  ...
  dhcp_options = {
    "${var.names_prefix}_dhcp_option" = {
      compartment_id     = null
      server_type        = var.dhcp_options.oci_ha_scheduler_dhcp_option.server_type
      search_domain_name = var.dhcp_options.oci_ha_scheduler_dhcp_option.search_domain_name
      forwarder_1_ip     = var.dhcp_options.oci_ha_scheduler_dhcp_option.forwarder_1_ip
      forwarder_2_ip     = var.dhcp_options.oci_ha_scheduler_dhcp_option.forwarder_2_ip
      forwarder_3_ip     = var.dhcp_options.oci_ha_scheduler_dhcp_option.forwarder_3_ip
    }
  }
}

module "oci_ha_scheduler_security_policies" {
  source = "github.com/oracle-terraform-modules/terraform-oci-tdf-network-security.git?ref=v0.9.7"

  default_compartment_id = var.default_compartment_id
  vcn_id                 = var.vcn_id

  nsgs = local.oci-ha-scheduler-nsg["${var.names_prefix}-nsg"]
}

module "oci_ha_scheduler_util_security_policies" {
  source = "github.com/oracle-terraform-modules/terraform-oci-tdf-network-security.git?ref=v0.9.7"

  default_compartment_id = var.default_compartment_id
  vcn_id                 = var.vcn_id

  nsgs = local.oci-ha-scheduler-util-nsg["${var.names_prefix}-util-nsg"]

}

module "oci_ha_scheduler_fss_sec_list" {
  source = "github.com/oracle-terraform-modules/terraform-oci-tdf-network-security.git?ref=v0.9.7"
...
module "oci_subnets" {
  ...
  subnets = {
    "${var.names_prefix}-subnet" = {
      ...
      freeform_tags     = var.freeform_tags
      dynamic_cidr      = false
      cidr              = var.oci_ha_scheduler_subnet_cidr
      cidr_len          = null
      cidr_num          = null
      enable_dns        = true
      ...
      security_list_ids = [data.oci_core_vcn.vcn.default_security_list_id, module.oci_ha_scheduler_fss_sec_list.security_lists["${var.names_prefix}-fss-sec-list"].id]
    }
  }
}
